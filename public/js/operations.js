...
(function () {
  ...
  function bindDumpButton() {
    ...
    function addItems(operations, hot) {

    //var base_url = "create/add/operations";
    console.log(base_url);
    
    $.ajaxSetup({ headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') } });
      $.ajax({
        url: base_url+"/discounts/operations/create/add/operations",
        type: "POST",  
        dataType: "json",
        data: { operations: operations},
        success : function(data) {
          console.log(data);
            var result = confirm(data.count  + "  Items Successfully Added!,.. Do you want to continue ? ");
            if(result) {
                 hot.clear();
                 return false;
            }
            else {
              window.location.href = base_url+"/discounts/operations";
            }
         },
