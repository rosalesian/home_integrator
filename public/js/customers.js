  ...
  function bindDumpButton() {
    ...
    Handsontable.Dom.addEvent(document.body, 'click', function (e) {
      ...
      if (element.nodeName == "BUTTON" && element.name == 'dump') {
        ...
        var size_data = hot.getData().length;

        if(size_data != 1) 
        {
          addItems(hot.getData(), hot);        
        }
        else 
        {
          alert('Please paste data in table');
        }
      }
    });
  }

  //sent data to items controller
  function addItems(customers, hot) {

    //var base_url = "create/add/customers";
   // console.log(base_url);
    
    $.ajaxSetup({ headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') } });
      $.ajax({
        url: base_url+"/discounts/customers/create/add/customers",
        type: "POST",  
        dataType: "json",
        data: { customers: customers},
        success : function(data) {
          //console.log(data);
            var result = confirm(data.count  + "  Customer Successfully Added!,.. Do you want to continue ? ");
            if(result) {
                 hot.clear();
                 return false;
            }
            else {
              window.location.href = base_url+"/discounts/customers";
            }
         },
